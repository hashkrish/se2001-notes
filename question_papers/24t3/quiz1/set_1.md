...
Which of the following actions will delete teh 7th line and save and exit in a `vi` editor or `emacs` editor? [MCQ]
...
DESCRIPTION
     ...
     -R hostname[:port]
             ...
             optional port number) from a known_hosts file. This option uses ssh-keygen's
             fail to delete hashed hosts (see the -H option above).
...
```
...
Select the **Regular Expression** to match the complete string of "address" (e.g. "address": "326-2072 Sagittis Road") from the below JSON file named `addresses.json`. [MSQ]
...
You have a large log file `server.log` and you want to count the number of occurrences of the word "ERROR" in the file. Which of the following command sequences will accomplish this? [MSQ]
...
You try to SSH into a remote VM using the command `ssh user@remote-vm` and recieve the following error:
...
In the 3rd and 4th, create links to each TARGET in the DIRECTORY.  
By default, `ln` makes hard links. Use `--symbolic` to make symbolic links.  
Symbolic links can link to directories, hard links cannot.
...
The format of a symbolic mode is `[ugoa]*([-+=]([rwxXst]*|[ugo]))+`.

Multiple symbolic modes can be separated by commas.

Each mode is applied in turn.  
...
Which of the following regular expressions (Extended Regular Expression) will match a MAC address in the format `00:1A:2B:3C:4D:5E`? [MSQ]
...
Given the following contents of a file `websites.txt`, what will be the output of the command `cat websites.txt | cut -d: f1 | sort | uniq | tail -l`? [SA]
